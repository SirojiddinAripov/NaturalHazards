...
function changeFontSize(mySize) {
  ...
  elements.forEach(function (element) {
    if (element.id === 'title') {
      element.style.fontSize = "".concat(mySize + 5, "px");
    } else {
      element.style.fontSize = "".concat(mySize, "px"); // Change font size for each element
    }
  });
  document.getElementById('fontSize').innerText = "Font Size: ".concat(mySize, "px");
}
var size = 20;
changeFontSize(size);
function listenForFontSize() {
  document.querySelector('#decreaseFontSize').addEventListener('click', function () {
    // eslint-disable-next-line no-unused-expressions
    size - 1 >= 12 ? changeFontSize(size -= 5) : changeFontSize(size);
  });
  document.querySelector('#increaseFontSize').addEventListener('click', function () {
    // eslint-disable-next-line no-unused-expressions
    size + 1 <= 40 ? changeFontSize(size += 5) : changeFontSize(size);
  });
}
listenForFontSize();
